...
def get_subtiles(tilename, epoch, consider_QA_rejected):
    ...
    if epoch[5] == "1":
        urlpath = urlopen(
            "https://vlass-dl.nrao.edu/vlass/quicklook/{}v2/{}".format(epoch, tilename)
        )
    else:
        urlpath = urlopen(
            "https://vlass-dl.nrao.edu/vlass/quicklook/{}/{}".format(epoch, tilename)
        )
    ...
    if consider_QA_rejected:
        ...
        if epoch[5] == "1":
            urlpath_rejected = urlopen(
                "https://vlass-dl.nrao.edu/vlass/quicklook/{}v2/QA_REJECTED".format(
                    epoch
                )
            )
        else:
            urlpath_rejected = urlopen(
                "https://vlass-dl.nrao.edu/vlass/quicklook/{}/QA_REJECTED".format(epoch)
            )
...
def search_vlass(c, crop=False, crop_scale=256, consider_QA_rejected=False):
    ...
    if len(glob.glob(imname)) == 0:
        if epoch[5] == "1":
            url_get = "https://vlass-dl.nrao.edu/vlass/quicklook/{}v2/{}/{}".format(
                epoch, tilename, subtile
            )
        else:
            url_get = "https://vlass-dl.nrao.edu/vlass/quicklook/{}/{}/{}".format(
                epoch, tilename, subtile
            )
        ...
        if not os.path.exists(subtile) and consider_QA_rejected:
            if epoch[5] == "1":
                url_get = "https://vlass-dl.nrao.edu/vlass/quicklook/{}v2/QA_REJECTED/{}".format(
                    epoch, subtile
                )
            else:
                url_get = "https://vlass-dl.nrao.edu/vlass/quicklook/{}/QA_REJECTED/{}".format(
                    epoch, subtile
                )
            fname = "{}{}".format(url_get, imname)
            subprocess.call("wget {}".format(fname), shell=True)
...
if __name__ == "__main__":
    ...
    if not glob.glob(summary_file_location):
        raise FileNotFoundError(
            errno.ENOENT, os.strerror(errno.ENOENT), summary_file_location
        )
